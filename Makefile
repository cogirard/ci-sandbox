...
export COMPOSE_FILE COMPOSE_PROJECT_NAME MAX_EXECUTION_TIME TIMEZONE SERVER_NAME MYSQL_DATABASE MYSQL_ROOT_PASSWORD
...
init:
	$(MAKE) composer.json build assets-build up fixtures fix-permissions
...
phpunit: ## Lance phpunit
	$(DOCKER_COMPOSE) exec -T php ./bin/phpunit
